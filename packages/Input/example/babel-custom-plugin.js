    ...
    visitor: {
      ...
      Program: {
        enter(path, { file }) {
          ...
          if (path.scope.hasBinding('Lodash')) {
            return;
          }

          const ourNode = t.importDeclaration([
            t.importDefaultSpecifier(t.identifier('Lodash')),
          ...
          const varDeclarationTest = t.variableDeclaration('var',
            [
              t.variableDeclarator(
                ...
                template.expression('styleGuard(SHEET_NAME, FILE_NAME)')({
                  SHEET_NAME: modifiedCSSModuleImportName,
                  FILE_NAME: JSON.stringify(value)
                })
              )
            ]
          );
