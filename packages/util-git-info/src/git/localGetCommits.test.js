...
test('generates a JSON-like commit message', async (t) => {
  t.deepEqual(formatJSON, '{ "sha": "%H", "parents": "%p", "author": {"name": "%an", "email": "%ae" }, "committer": {"name": "%cn", "email": "%ce" }, "subject": "%s", "sanitizedSubject": "%f", "body": "%b", "authoredOn": "%aI", "committedOn": "%cI", "body": "%b"},')
